...
- Reply layout: one collapsed process block per assistant message, followed only by the final round's answer. Inside the process block, show sequential groups without round numbers with expandable thinking and individual tool calls; intermediate answers are plain text within their round. Never duplicate the final answer inside the process block. Omit the context/source/token summary from the process body; these details remain in the inspector.
- Process summary: start with “准备中”; while running show only the current action. Completion shows “已完成 · 用时 …”; use measured time for simulated executions. No round numbers or context/source/token line.
- Process presentation follows a continuous transcript: inline muted text and an adjacent chevron, one thin divider, no background fills on disclosure headers. Expanded thinking and tool output share the same subtle output surface. Intermediate replies use the body size and primary text color. Group consecutive tool calls into one expandable action summary; each entry is named after the tool and reveals only its returned content inside a subtle `--content-bg-2` block, without repeating the tool name or input arguments. Only the final reply appears outside the transcript. Transcript items, expanded details, and tool entries share a compact `--space-1` vertical gap; step headers use the natural UI line height with no vertical padding or minimum control height, and round wrappers add no spacing.
- The outer transcript opens during execution and collapses on completion/stop. Thinking, tool groups, and tool details open when active and collapse when finished. Users can manually revisit completed sections. Do not render future steps or fabricate completed output after stopping.
...
- User message bubbles use the subtle `--content-bg-2` surface rather than the stronger active-state fill.
