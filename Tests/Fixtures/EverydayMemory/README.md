...
Each scenario also declares `requiresCitation` as part of its handoff identity. Only `zh-writing-plan` sets it to `true`; its fresh-session recall must include a non-empty verified canonical memory citation. Every canonical citation visible in either phase is checked through the production memory citation authorization path.
...
The launcher in `scripts/run_memory_continuity.py` defaults to eight credential authorizations per case across both phases, with a six-authorization establishment ceiling and 32 overall. An authorized targeted follow-up can select known cases with repeatable `--case-id` and reduce the per-case allocation with `--case-cap` (4–8); establishment reserves at most `cap - 2`, leaving two for recall. It preserves failure reports and process exit evidence, refuses output-directory reuse, and does not automatically retry or borrow another case's allocation. An explicit one-time launcher recovery imports prior actual counts before allocating the remainder; it requires all four cases at cap 8 and cannot reset the task budget. Model reply and stored-fact semantics require separate review. See the [testing workflow](../../../docs/engineering/EVERYDAY_MEMORY_TESTING.md) for invocation and limits.
