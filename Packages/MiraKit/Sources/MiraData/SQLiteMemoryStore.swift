...
public final class SQLiteMemoryStore: MemoryStore, MemoryCapturePolicyStore, @unchecked Sendable {
    ...
    public func memoryCapturePolicy() async throws -> MemoryCapturePolicy { try await owner.read { try Self.currentCapturePolicy(in: $0) } }
    public func saveMemoryCapturePolicy(_ policy: MemoryCapturePolicy, expectedRevision: Int, authorization: AgentLibraryAuthorization, at: Date) async throws {
        try policy.validate(); try Self.date(at)
        try await owner.write(authorization: authorization) { db in
            let previous = try Self.currentCapturePolicy(in: db)
            guard previous.revision == expectedRevision, expectedRevision < Int.max, policy.revision == expectedRevision + 1 else { throw Self.conflict }
            try db.execute(sql: "UPDATE memory_policy SET revision = ?, json = ? WHERE id = 1", arguments: [policy.revision, try Self.encode(policy)])
            try SQLiteMemoryExtractionStore.invalidatePolicy(at: at, in: db)
        }
    }
    ...
    static func currentCapturePolicy(in db: Database) throws -> MemoryCapturePolicy {
        guard let row = try Row.fetchOne(db, sql: "SELECT * FROM memory_policy WHERE id = 1") else { throw corrupt }
        let value: MemoryCapturePolicy = try decode(row["json"])
        do { try value.validate() } catch { throw corrupt }
        guard value.revision == row["revision"] as Int else { throw corrupt }
        return value
    }
}
