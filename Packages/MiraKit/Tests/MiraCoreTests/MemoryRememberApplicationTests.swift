...
struct MemoryRememberApplicationTests {
    @Test func directRememberCommitsLocalMemoryAndPairedReceiptBeforeFinalReply() async throws {
        ...
        let retainedReply = try #require(afterForget.messages.first { $0.role == .assistant })
        #expect(retainedReply.bodyPurgedAt == nil && retainedReply.text == "Saved your preference.")
        #expect(retainedReply.trace.allSatisfy { $0.role == .assistant && $0.toolCalls == nil && $0.toolCallID == nil })
        #expect(afterForget.memoryNotices[executionID] == [.init(memoryID: memory.id, reason: .forgotten)])
