...
private struct KnowledgeFixture: KnowledgeReadStore, Sendable {
    ...
    private var evidence: SessionUserEvidence { let sid = ConversationID(); let bid = UUID(); _ = SessionContent(id: UUID(), kind: .userText, bytes: Data("notes".utf8)); return .init(reference: .init(sessionID: sid, originalExecutionID: ExecutionID(), userMessageID: MessageID(), admissionEventID: UUID(), admissionSequence: 1), workspaceID: nil, admittedAt: Date(timeIntervalSince1970: 1), timeZoneIdentifier: "UTC", text: "notes", observedHead: .init(cursor: .init(sessionID: sid, sequence: 1), batchID: bid), sessionAuthorizationEpoch: 0) }
