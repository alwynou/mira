...
private struct KnowledgeFixture: KnowledgeReadStore, Sendable {
    ...
    private var evidence: SessionUserEvidence { let sid = ConversationID(); let bid = UUID(); let body = SessionPayloadReference(id: UUID(), sessionID: sid, batchID: bid, retentionGroup: UUID(), kind: .userText, byteCount: 1, digest: String(repeating: "0", count: 64)); return .init(reference: .init(sessionID: sid, originalExecutionID: ExecutionID(), userMessageID: MessageID(), admissionEventID: UUID(), admissionSequence: 1, body: body), workspaceID: nil, admittedAt: Date(timeIntervalSince1970: 1), timeZoneIdentifier: "UTC", text: "notes", observedHead: .init(cursor: .init(sessionID: sid, sequence: 1), batchID: bid), sessionAuthorizationEpoch: 0) }
    private var route: AgentModelRoute { .init(id: RouteID(), revision: 1, connectionID: ConnectionID(), connectionRevision: 1, modelDescriptorID: ModelDescriptorID(), modelRevision: 1, modelAuthorizationRevision: 1, adapter: .init(id: "test", revision: 1), invocationID: "test-invocation", invocationRevision: 1, endpointID: "test-endpoint", metadataEvidence: [], modelID: "test", credential: nil, contextWindow: 4096, maximumOutputTokens: 128, capabilities: .init(streamsText: true, callsTools: false, producesThinking: false), configuration: .object([:])) }
