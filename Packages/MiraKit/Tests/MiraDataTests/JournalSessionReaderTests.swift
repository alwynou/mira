...
struct JournalSessionReaderTests {
    @Test("Extraction derives the last batch turn from canonical records and rejects mismatched scope")
    func extractionUsesLastBatchTurn() async throws {
        let outputs: [[AgentModelStreamEvent]] = ["First answer", "Last answer"].map { text in
            [.blockStarted(.init(id: "answer", content: .text(text))), .blockFinished(id: "answer"), .finished(.stop)]
        }
        try await withTaskWorkflow(outputs: outputs) { fixture in
            let first = try await fixture.run("First synthetic preference")
            let last = try await fixture.run("Last synthetic preference", sessionID: first.sessionID)
            let reader = JournalSessionReader(journal: fixture.library, payloads: fixture.library)
            var turns: [MemoryExtractionTurn] = []
            for address in [first, last] {
                let evidence = try await fixture.evidence(address)
                let batches = try await fixture.library.read(sessionID: address.sessionID, after: 0, limit: 128)
                let batch = try #require(batches.first { batch in
                    batch.events.contains { event in
                        if case .finished(let completion) = event.fact { return completion.executionID == address.executionID }
                        return false
                    }
                })
                let event = try #require(batch.events.first { event in
                    if case .finished(let completion) = event.fact { return completion.executionID == address.executionID }
                    return false
                })
                turns.append(.init(source: evidence.reference, completedExecutionID: address.executionID,
                    completionEventID: event.id, completionHead: .init(cursor: batch.cursor, batchID: batch.id),
                    admittedAt: evidence.admittedAt, completedAt: event.occurredAt, inputTokenEstimate: 10))
            }
            let origin = MemoryExtractionOrigin(source: turns[0].source,
                completedExecutionID: first.executionID, completionEventID: turns[0].completionEventID,
                completionHead: turns[0].completionHead)
            let job = MemoryExtractionJob(id: .init(), origin: origin, workspaceID: nil,
                createdAt: TaskWorkflowFixture.now, updatedAt: TaskWorkflowFixture.now, turns: turns)
            let prefix = try await reader.memoryExtractionContext(for: job)
            #expect(prefix.route == fixture.route)
            #expect(prefix.input.executionID == last.executionID)
            #expect(prefix.input.instructions == "Use the available task tools.")
            #expect(prefix.input.messages.last?.text == "Last synthetic preference")
            #expect(!prefix.input.messages.contains { $0.text == "First synthetic preference" })
            let forged = MemoryExtractionJob(id: .init(), origin: origin, workspaceID: .init(),
                createdAt: TaskWorkflowFixture.now, updatedAt: TaskWorkflowFixture.now, turns: turns)
            await #expect(throws: MiraError.self) { _ = try await reader.memoryExtractionContext(for: forged) }
        }
    }
