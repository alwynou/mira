...
struct SessionProjectionTests {
    ...
    @Test func messagePageLinksRowsIncludesActiveExecutionAndPaginatesAtomically() async throws {
        ...
        do {
            ...
            #expect(second.executions.map(\.id) == [retry.executionID])
...
private final class ProjectionFixture: @unchecked Sendable {
    ...
    func openSession(title: String, workspaceID: WorkspaceID? = nil, finish: ExecutionStatus? = nil,
        ...
        try requireCommitted(await runtime.commit(id: UUID()) { context in
            ...
            let plan = try await context.stage(AgentExecutionPlan(runtimeID: UUID(), catalogGeneration: 1, driverID: "mira.default", driverRevision: 1, instructions: "", limits: .init(), priority: .foreground, route: nil), kind: .executionPlan)
        ...
        if let finish {
            try requireCommitted(await runtime.commit(id: UUID()) { context in
                ...
                let thinking = try await context.stageBytes(Data("Thinking".utf8), kind: .visibleThinking)
                let error = finish == .completed ? nil : try await context.stageBytes(
                    SessionCodec.encode(JSONValue.string("Hidden error")), kind: .error)
    ...
    func retry(session: SessionFixture) async throws -> SessionFixture {
        ...
        try requireCommitted(await runtime.commit(id: UUID()) { context in
            let plan = try await context.stage(AgentExecutionPlan(runtimeID: UUID(), catalogGeneration: 1, driverID: "mira.default", driverRevision: 1, instructions: "", limits: .init(), priority: .foreground, route: nil), kind: .executionPlan)
            return [
                .admitted(.init(executionID: retryID, userMessageID: session.userMessageID,
                                retryOfExecutionID: session.executionID, userBody: nil, plan: plan,
                                hasModelRoute: false, authorizationEpoch: 0, timeZoneIdentifier: "UTC")),
                .retrySuperseded(.init(sourceExecutionID: session.executionID, retryExecutionID: retryID)),
            ]
        })
    ...
    func finish(executionID: ExecutionID, status: ExecutionStatus) async throws {
        try requireCommitted(await runtime.commit(id: UUID()) { context in
            let answer = try await context.stageBytes(Data("Answer".utf8), kind: .visibleAnswer)
            let thinking = try await context.stageBytes(Data("Thinking".utf8), kind: .visibleThinking)
            let error = status == .completed ? nil : try await context.stageBytes(
                SessionCodec.encode(JSONValue.string("Hidden error")), kind: .error)
    ...
    func finishWithoutOutput(executionID: ExecutionID, status: ExecutionStatus) async throws {
        try requireCommitted(await runtime.commit(id: UUID()) { context in
            let error = try await context.stageBytes(
                SessionCodec.encode(JSONValue.string("No visible output")), kind: .error)
...
private extension SessionFact {
    var titleReference: SessionContent? { if case .opened(let header) = self { return header.title }; return nil }
}
