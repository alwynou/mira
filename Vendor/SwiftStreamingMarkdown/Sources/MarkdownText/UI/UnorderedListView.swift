...
struct UnorderedListView: View {
        ...
        HStack(alignment: .centerOfFirstLine, spacing: 1) {
          ...
          if let firstChild = items[idx].children.first {
            if case .paragraph(_, let contents) = firstChild {
              ...
              .accessibilityLabel(Text(listItemAccessibilityLabel(for: contents.string, at: idx, checkbox: items[idx].checkbox, locale: locale)))
            } else {
              SingleBlockView(renderable: firstChild)
            }
          }
  ...
  private func listItemAccessibilityLabel(for content: String, at index: Int, checkbox: MarkdownListItem.Checkbox?, locale: Locale? = nil) -> String {
    let label = markdownListAccessibilityLabel(for: content, at: index, length: items.count, locale: locale)
    switch checkbox {
    case .checked: return "\(label), \(String.taskListItemChecked(locale: locale))"
    case .unchecked: return "\(label), \(String.taskListItemUnchecked(locale: locale))"
