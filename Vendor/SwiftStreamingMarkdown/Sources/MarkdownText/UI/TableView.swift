  ...
  private func headerView(colIdx: Int) -> some View {
    HStack(spacing: 0) {
      Text(headings[colIdx])
        ...
        .accessibilityValue(String.itemPositionInTable(rowIndex: 1, totalRow: numOfRows + 1, columnIndex: colIdx + 1, totalColumn: headings.count, locale: locale))
      Spacer()
    }
    .padding(12)
    .id("\(colIdx)-heading")
    ...
    case .containsAttachment(let nsAttributedString):
      HStack(spacing: 0) {
        ParagraphView(contents: applyTypographyThemingAndGetContent(nsAttributedString))
          ...
          .accessibilityValue(String.itemPositionInTable(rowIndex: rowIdx + 2, totalRow: numOfRows + 1, columnIndex: colIdx + 1, totalColumn: headings.count, locale: locale))
        Spacer()
      }
