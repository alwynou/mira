...
enum CodeViewConfiguration {
    ...
    extension CodeView {
        ...
        private func setupScrollView() {
            ...
            scrollView.setAccessibilityIdentifier("markdown.codeBlock")
            ...
            NotificationCenter.default.addObserver(self, selector: #selector(syncLineNumberOffset),
        ...
        private func setupLineNumberView() {
            ...
            lineNumberClipView.drawsBackground = false
            lineNumberClipView.documentView = lineNumberView
            addSubview(lineNumberClipView)
            updateLineNumberView()
        }
        ...
        private func layoutLineNumberView(barHeight: CGFloat) {
            let lineNumberSize = lineNumberView.intrinsicContentSize
            lineNumberClipView.frame = CGRect(
                x: 0,
                y: barHeight,
                width: lineNumberSize.width,
                height: max(0, bounds.height - barHeight)
            )
            lineNumberView.frame = CGRect(origin: .zero, size: lineNumberSize)
        }

        @objc func syncLineNumberOffset() {
            lineNumberClipView.scroll(to: CGPoint(x: 0,
                y: scrollView.contentView.bounds.minY + CodeViewConfiguration.codePadding))
        }
        ...
        private func layoutScrollViewAndTextView(barHeight: CGFloat) {
            ...
            textView.frame = CGRect(
                x: 0,
                y: 0,
                // Pad the document, not contentInsets: AppKit moves overlay scrollers
                // inward with contentInsets, which would still cover the last glyphs.
                width: max(scrollView.bounds.width - CodeViewConfiguration.codePadding * 2,
                           textContentSize.width + CodeViewConfiguration.scrollerClearance),
                height: textContentSize.height + CodeViewConfiguration.scrollerClearance
            )
        }
    }
#endif
