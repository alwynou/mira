    ...
    final class CodeView: NSView {
        ...
        var content: String = "" {
            didSet {
                ...
                cachedLineCount = max(content.components(separatedBy: .newlines).count, 1)
                textView.attributedText = highlightMap.apply(to: content, with: theme)
                textView.setAccessibilityValue(content)
                lineNumberView.updateForContent(content)
                updateLineNumberView()
            }
        }
        ...
        lazy var scrollView: NSScrollView = {
            ...
            sv.allowsVerticalScrolling = true
        ...
        lazy var lineNumberView: LineNumberView = .init()
        let lineNumberClipView = NSClipView()
        ...
        func interactionTarget(at point: CGPoint) -> NSView? {
            ...
            // The gutter and blank code padding belong to the same scroll surface.
            if point.y >= barView.frame.maxY { return scrollView }
            return nil
        }
        ...
        override var intrinsicContentSize: CGSize {
            ...
            return CGSize(
                ...
                height: min(
                    max(barHeight + textSize.height + scrollView.contentInsets.top + scrollView.contentInsets.bottom
                        + CodeViewConfiguration.scrollerClearance, supposedHeight),
                    max(barHeight + CodeViewConfiguration.codePadding * 2 + theme.fonts.code.pointSize,
                        theme.maximumCodeBlockHeight)
                )
            )
        }
        ...
        func updateLineNumberView() {
            ...
            lineNumberView.padding = NSEdgeInsets(
                top: CodeViewConfiguration.codePadding,
                left: CodeViewConfiguration.lineNumberPadding,
                bottom: CodeViewConfiguration.codePadding + CodeViewConfiguration.scrollerClearance,
                right: CodeViewConfiguration.lineNumberPadding
            )
        }
    }
