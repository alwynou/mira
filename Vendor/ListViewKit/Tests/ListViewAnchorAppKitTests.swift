...
struct ListViewAnchorAppKitTests {
    ...
    /// Native scrolling clamps to the new document extent when rows disappear.
    /// Row opacity/frame animation must not leave the native clip outside it.
    @Test
    func anAnimatedApplyKeepsTheNativeOffsetInsideTheNewDocument() {
    ...
    func compensationPastAnEdgeSettlesEvenWhenTheHeightIsUnchanged() {
        let context = makeContext()
        let listView = context.listView
        let heightBefore = listView.listContentSize.height
        ...
        #expect(listView.listContentSize.height == heightBefore)
        #expect(listView.contentOffset.y == listView.minimumContentOffset.y)
    }
