...
public final class ListView<Item: Identifiable & Hashable & SendableMetatype>: ListScrollView {
    ...
    public func apply(_ newItems: [Item], animated: Bool = false) {
        ...
        // Row appearance animates independently of the native clip's bounds.
        // AppKit keeps the viewport inside the current document extent.
        withListAnimation {
    ...
    var contentRect: CGRect {
        .init(x: 0, y: 0, width: viewportSize.width, height: rowLayout.contentHeight)
    }
    ...
    override public func layoutContent() {
    ...
    private func settleNewlyPlacedRows() {
        ...
        withoutListAnimation {
            for view in pending where view.superview === rowContainer {
                view.layoutNow()
            }
        }
    }
    ...
    func context(at index: Int, purpose: ListRowPurpose) -> ListRowContext {
        .init(index: index, width: viewportSize.width, purpose: purpose)
    }
    ...
    func prototype(for registrationIndex: Int) -> Prototype {
        ...
        rowContainer.addSubview(view)
        let width = view.widthAnchor.constraint(equalToConstant: viewportSize.width)
        // Position is pinned only so Auto Layout has no ambiguity to warn
        // about; nothing ever reads this view's origin.
        NSLayoutConstraint.activate([
            width,
            view.topAnchor.constraint(equalTo: rowContainer.topAnchor),
            view.leadingAnchor.constraint(equalTo: rowContainer.leadingAnchor),
        ])
    ...
    private func ensureRowView(at index: Int) {
        ...
        if view.superview !== rowContainer {
            rowContainer.addSubview(view)
        }
    }
