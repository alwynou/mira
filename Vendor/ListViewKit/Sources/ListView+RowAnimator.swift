...
extension ListView {
    ...
    private func animatorContext(scrollDelta: CGFloat, deltaTime: TimeInterval) -> ListAnimatorContext {
        if let live = interactionLocationInViewportY, viewportSize.height > 0 {
            rowAnimatorGripViewportY = min(max(0, live), viewportSize.height)
        }
        let viewport = viewportRect.offsetBy(dx: 0, dy: topInset)
        return .init(
            viewportRect: viewport,
            contentRect: contentRect.offsetBy(dx: 0, dy: topInset),
            interactionAnchorY: viewport.minY + (rowAnimatorGripViewportY ?? viewportSize.height),
