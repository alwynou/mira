...
Forget 会清理 Memory 正文、修订版本、Evidence 正文、工具 / 请求 / 审计缓存和活跃草稿，并抑制其来源的自动提取。已提交的历史用户消息、Assistant 回复与可显示 Trace（Assistant 正文和可显示思考）仍在本地可见，并显示不含正文的状态标签；Trace 中隐藏的工具消息、参数、结果和 Tool Call 标识会被清理。这些历史内容及其传递后代不再用于未来 Provider Context，也不被模型重放。原始来源消息可以保留，但不因此恢复模型重放资格。
...
首个接收真实长期数据的版本必须支持可用的本地导出与恢复路径，而非只导出不可重新导入的文本。恢复保留稳定对象 ID、来源版本与关联，不包含模型凭据，不自动执行历史任务、重新发布到 Apple 或再次启用被用户关闭的后台操作。恢复遗忘后导出的备份时，保持其清理状态和来源抑制，不从保留的历史回复重新构造 Memory、缓存或已删除的正文。遗忘前导出的旧备份可能仍含原内容；Mira 不会改写用户此前的导出文件。
