...
The conversation does not display a memory extraction disclosure, recall citation badge, historical memory tag, or automatic memory status panel. Assistant answer presentation omits internal memory reference markup and known memory receipt IDs, including during streaming; Knowledge citations and explicit deletion outcomes remain available. Reply/archival notices join the measured floating bottom region. Automatic extraction remains controlled by Memory settings; its replacement feedback presentation is deferred. The optional execution inspector can show scoped extraction job status and per-attempt accounting using existing labels, disclosure groups, and neutral typography. This does not add a transcript status panel or shared tokens.
