...
Color values are appearance-aware. They resolve against Mira's selected display mode; Follow System inherits the current macOS appearance.
...
`MiraSettingsPage` supplies the same `canvas` background as conversation content and the existing reading-width cap; the status footer uses that canvas too. Providers and Models show the footer only for operation status or errors, without a persistent demo-mode notice or empty footer space. `MiraSettingsSection`, `MiraSettingsRow`, `MiraSettingsDivider`, and `MiraSettingsSearchField` compose groups using the existing surface, border, typography, spacing, and radius tokens. Panels use the shared `panel` radius and navigation uses the shared `row` radius. They introduce no separate settings palette or corner treatment. Native pickers, menus, disclosure groups, secure fields, and buttons retain keyboard and accessibility behavior. General uses `MiraSettingsSelect` for display language: a 28 pt inset-filled trigger with a 9 pt radius, quiet border, current value, and chevron. The trigger and menu measure their intrinsic content width independently. `minimumWidth` and `menuMinimumWidth` default to `Layout.selectMinWidth` (100 pt) and `Layout.selectMenuMinWidth` (140 pt); `maximumWidth` and `menuMaximumWidth` are optional and impose no cap when omitted. General configures the respective maximums to `Layout.selectMaxWidth` (200 pt) and `Layout.selectMenuMaxWidth` (320 pt). Short labels retain the minimum, longer content expands until the configured maximum, and the trigger truncates overflowing text at the tail. Menu options instead wrap at the capped content width and grow vertically. Both use the same 14 pt body type; single-line menu rows and the trigger are 28 pt high, with 4 pt vertical padding inside menu rows for multiline breathing room. Menu width follows its longest option; trigger width follows the current label, so locale or selection changes may change either width within its bounds. Widths include padding and icons. Hover, keyboard focus, and expansion change the fill only; the border retains the normal `border` token in both appearances, including dark mode. Increased Contrast can strengthen the border independently of activation. A surface-filled menu contains padded option rows with a selected highlight and checkmark. The menu shares the trigger's 9 pt row radius with no pointer, a quiet border, and a compact black shadow at 12% opacity, 4 pt radius, and 2 pt vertical offset. `MiraSettingsPage` renders it above scrolling content from the trigger anchor, aligned to the trailing edge with a 4 pt gap; it does not use native popover chrome. Tab/Shift-Tab remain within its options while it is open. Option buttons suppress the native blue focus ring; keyboard focus uses the same single neutral row fill as hover, without changing selection semantics. `MiraSidebarRow.isKeyboardFocused` provides that treatment without stacking highlight layers. Its `minimumHeight` override lets menu rows share the 28 pt control height; navigation rows retain the default 34 pt height. Interaction fills and increased-contrast outlines reuse the existing color tokens; no motion or transparent custom backdrop is added. Arrow keys navigate, Return selects, and Escape or clicking the page outside the menu dismisses without changing the value. Pointer selection and clicking outside the menu clear stale hover and trigger focus when closing. Escape and keyboard selection return focus to the trigger with its neutral focus fill. Leaving the category unmounts the menu. Selection applies immediately through the existing saved language preference.

Every category page begins with `MiraSettingsHeader`: the category name in the existing 20 pt semibold `title` style, followed by a concise description in the 12 pt `caption` style and `secondaryText` color. The two lines retain `sm` (8 pt) spacing. Below the subtitle, the header adds `sm` to the page's `xl` group spacing, placing the first content block 32 pt below the subtitle; subsequent groups retain 24 pt spacing. The title has the accessibility heading trait, and descriptions can wrap at narrow widths without clipping. General describes display language and appearance; Providers describes provider connections and models; Models describes defaults and the model pool; Memory describes automatic memory and extraction limits; Data & Privacy describes local storage, backups, and cleanup. Copy lives in the English/Simplified Chinese string catalog. Providers keeps Add Provider beside its header, and individual provider details retain their provider name and endpoint heading.
