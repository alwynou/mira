...
The model pool contains enabled models belonging to active providers. Context and capability information can remain unknown while configuring a model; sending remains blocked until the selected purpose's requirements are met. Conversation and default-purpose selectors filter this pool by the requirements of their purpose and identify both model and provider. Management retains models that are not ready and explains their missing capabilities or unsupported protocol. The pool can be filtered for conversation, Agent tools, and memory extraction. The first enabled model admitted to the pool under an active provider initializes the conversation default if it is absent. Later models do not replace an existing default. Pool membership never authorizes background memory extraction.

Conversation settings offer a fixed default model or **Follow last selected model**. A fixed default initializes new conversations; changing the current conversation's model leaves that default unchanged. Follow-last uses the latest explicit model choice in this library, including a validated choice in an unsent draft. Without a remembered choice it uses the configured fixed default. Opening an existing conversation does not change the remembered choice. New conversations commit their concrete selection with the first message, so later defaults do not retarget them. The composer shows the actual model name and opens a flat list with small provider headings and a checkmark for the current model. There is no "Use default model" entry in this conversation list. Local follow-last preferences survive app restarts; they do not authorize background work.
...
当前版本在对话执行详情展示前台费用，在记忆提取状态展示后台费用，并保留逐次调用明细。任一调用费用未知时，总额显示未知，同时列出已知部分小计和未知调用数。缺失缓存用量、无适用价格或流式调用未完整结束时，不显示看似完整的金额。价格来源版本与获取时间可见；修改模型设置不会重算历史价格。
