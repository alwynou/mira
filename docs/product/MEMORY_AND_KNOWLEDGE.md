...
- “忘记这条”立即阻止这条 Memory 继续召回和后台再提取，并清理包含该 Memory 的工作记忆、摘要与请求缓存等派生副本。
...
更新 Source 或删除来源时，已人工确认的 Memory 不静默消失或重新解释为新版本资料的结论。显示其依据的原始版本或“来源已不可用”状态。若 Evidence 存在归属错误或不能再支撑该记忆，退出自动事实注入并等待纠正。
