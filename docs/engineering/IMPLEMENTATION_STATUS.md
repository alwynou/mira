...
| M1 对话 | Workspace / Inbox、项目背景与发送开关、对话创建 / 归档、流式文本、取消、最后失败回合重试、本地历史与草稿恢复 |
| M1 Provider | OpenAI Chat Completions 兼容接口与 Anthropic Messages、手工 Model ID / 窗口 / 输出上限、明确流式文本能力声明、Keychain 引用与版本 |
...
当前没有记忆保存、Markdown 导入检索或工具执行入口。系统提示也明确当前能力，避免文本对话声称已执行尚未实现的操作。
...
1. 增加用户主动触发、只发送固定合成文本的连接探测，持久化 observed capability / 失败原因；当前仅支持用户声明流式文本能力，未知窗口会在发送前阻止请求。
...
这些条目使 M1 保持“进行中”。它们没有被从规范中删除；完成后再进入 M2 完整 Step / Attempt、工具交换、Context Inspector 与资源预算。

## 4. 实现范围与设计目标的关系

- 当前 Execution 只实现无工具文本回合需要的状态。每次执行一次 ModelCall，以 Execution ID 存储唯一请求；M2 将引入独立 Step / Attempt / Snapshot，不将此阶段的一次请求结构当作最终多步设计。
- 当前输入预算采用 UTF-8 字节数、协议开销与安全余量的保守估算，超预算提示新建对话，不静默截断或伪造精确 Tokenizer。
- 初版 Schema 只覆盖已实现实体。Message 使用受约束的纯文本列；Typed Parts、Blob、Memory、Source 等按实际里程碑迁移加入。
...
未完成真实 Provider / Keychain 演练、macOS 15 本机 UI、Intel、7 天使用、完整记忆质量评估、Blob 恢复、签名、公证或下载包安装升级测试。CI 在 macOS 15 上执行包测试和应用编译也不等同于这些 UI 与发布验收。当前版本仅作为本机开发增量。
