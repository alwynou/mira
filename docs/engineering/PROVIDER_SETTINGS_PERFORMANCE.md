...
- The immutable catalog indexes normalized protocol/endpoint keys and model IDs without changing exact matching, regional metadata or catalog order. Providers reads configuration without enumerating routing scopes; Models still loads them. Identical editor inputs reuse prepared candidates. Test/save/activation/send validation remains local and revision-checked; enabling no longer requires a network probe.
...
## Activation toggle follow-up

Provider enablement is a local state change. A nonempty entered or stored key is enough; no test model or successful network request is required. `saveConnection` no longer accepts a test-model argument or invokes the probe. Only the explicit Test action or model use checks credentials. A failed explicit test does not change the saved enabled state, and enabling never shows a connectivity-success message.

The disabled/enabled binding previously read only the persisted baseline, delaying switch feedback while an operation was pending. The save's own configuration event could also reach the editor before `saveConnection` returned, incrementing the draft generation and causing a successful save result to be discarded. The notification-before-return regression failed before the fix and passes afterward. The editor now holds a pending switch value and reconciles notifications after persistence. Save failure/cancellation restores the persisted state; a newer external revision wins with a conflict, and unsaved input is preserved. Repeated requests for the current state do not write.

Configuration mutations emit `configurationChanged` after the existing transaction and cancellation boundaries. Conversation pages update available routes without rereading cached history or sidebar scopes. Provider observers ignore draft/thinking events, serialize overlapping reads and skip a saved revision already installed by its event. Connection validation and cleanup read configuration directly; unchanged credential references/versions reuse the session's already-loaded secure value. Model attestations, frozen-route checks and memory-cancellation ordering remain unchanged.

Verification:

- Package: 394 tests in 43 suites passed for the scoped runtime event change; `/tmp/mira-provider-toggle-package-final.log`. The later activation-contract change does not modify package code.
- Activation-contract build and host tests passed: 82 Swift Testing tests in 15 suites and 27 XCTest cases, with one opt-in live case skipped. Evidence: `/tmp/mira-provider-local-toggle-host.log`. Cases cover enablement with an unverified key and no test model, rejection of an empty key, explicit-test failure without disabling, notification-before-return, cancellation, failed save, newer external revisions, unchanged credential reads, and configuration updates without cached-history reads.
- A native `NSSwitch.performClick` fixture hosts the production editor at 552 × 560 pt with synthetic storage. It blocks only local persistence, checks immediate switch feedback with no testing status, then completes activation and deactivation with zero probe calls and one credential read. Light/dark pending-feedback layout measured 16.76 / 14.85 ms in the accepted Debug run. These are synthetic control/layout timings, not real provider, database or Keychain latency.
- Native editor captures were inspected: `/tmp/mira-provider-toggle-pending-en.png`, `/tmp/mira-provider-toggle-disabled-en.png`, `/tmp/mira-provider-toggle-pending-zh-CN.png`, `/tmp/mira-provider-toggle-disabled-zh-CN.png`. Host-bundle strings render in English even with a Chinese locale; actual translated app screenshots are verified separately.
- Language policy passes for 1,179 bilingual strings. The generated project includes the production editor in the host test target; whitespace checks pass.

Both complete settings UI workflows passed in disposable offline libraries: Chinese/dark and English/light at minimum size, including provider selection, scrolling, retained drafts and close/reopen reset. The revised bilingual explanation was visually verified. Result: `.build/xcode/Logs/Test/Test-MiraUI-2026.09.12_17-47-50-+0800.xcresult`; log: `/tmp/mira-provider-local-toggle-ui.log`; captures: `/tmp/mira-provider-local-toggle-images`. No real credential or paid endpoint is used in these fixtures. macOS 15 runtime behavior, real Keychain latency and display-frame profiling remain separate acceptance checks.

## Missing-key feedback

An empty-key provider switch remains clickable. An enable attempt leaves the persisted and displayed state off, performs no save or probe, and focuses the native secure input. The shared credential component draws a system-red outline with an inline localized required-key message and an accessibility hint. Nonempty input clears this feedback immediately; discard and settings closure reset it. Stored-key reuse and local-only activation are unchanged. The component gallery includes the error state.

The production editor's native fixture passed in light/dark appearance at 552 × 560 pt. It presses the actual `NSSwitch`, asserts the switch remains off, verifies that the secure field owns the native first responder, types through its `NSTextView`, checks the error clears, and successfully enables with zero probe calls. Captures were inspected at `/tmp/mira-provider-key-required-en.png`, `/tmp/mira-provider-key-required-zh-CN.png` and the corresponding `mira-provider-key-entered-*.png` paths. As above, the host fixture renders English strings; the localized Simplified Chinese message is covered by the catalog/explicit-locale tests.

Final build and host tests passed: 82 Swift Testing tests in 15 suites and 28 XCTest cases, with one opt-in live case skipped. Evidence: `/tmp/mira-provider-key-required-accepted.log`. Language policy passes for 1,179 bilingual strings. No real credentials, database schema changes or provider calls are part of this increment. The earlier complete settings UI workflows were not repeated for this field-validation change; native control, focus, secure text editing and localization are the directly verified boundaries.
