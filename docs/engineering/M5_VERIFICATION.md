...
Final M5 package/CI/artifact evidence is added after those checks complete. Native picker/citation/keyboard/VoiceOver/language-switching walkthroughs, cold UI startup, streaming-main-thread profiling, and cancel-feedback timing remain dependent on a functioning native automation service or attended environment. Real providers, attended Keychain faults, human-labeled Q04–Q06 data, seven-day use, minimum-system/additional-CPU native acceptance, Developer ID signing, and notarization remain deferred in the [execution ledger](MVP_EXECUTION.md).
