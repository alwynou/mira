...
The corpus's required `requiresCitation` flag is part of process handoff identity. The Chinese ordinary follow-up requests a source citation. Both phases reject malformed visible memory references and resolve canonical ones using production source authorization; a required-citation recall also fails if no citation resolves. Saving a memory does not alone authorize citing its receipt reference.
