...
Date: 2026-09-17. Status: historical plan, implemented with the current DSH settlement boundary. Acceptance and deviations are tracked in [verification](AGENT_SESSION_REFACTOR_VERIFICATION.md) and the authoritative [session log](../architecture/AGENT_SESSION_LOG.md).
...
> The initial plan below contains historical alternatives for durable draft checkpoints. The accepted implementation follows the process-local settlement section in `AGENT_SESSION_REFACTOR_VERIFICATION.md`; those alternatives are retained only to explain the design decision.
...
DSH is the reference for event content, message derivation, header folding, and turn/step orchestration. Port these responsibilities into Swift without adding a TypeScript implementation dependency. Mira retains atomic input admission and process-local streams until settlement. DSH's released-format migrations and broader dynamic prompt/series capabilities remain outside the current scope.
...
| `assistant/attempt` and settled stream records | Failed/cancelled attempt state and any output settled at the terminal boundary; do not masquerade as a successful answer |
...
Follow DSH's distinction between assembled message content and the compact per-attempt stream: the message is the model-history input, while the stream preserves attempt diagnostics and protocol detail. Keep stream records nested under the settled assistant message/attempt instead of creating a top-level log row for every delta. Mira's stream remains process-local until settlement; unresolved output is intentionally lost on hard crash. Avoid additional full-turn replay copies and repeated full request bodies. Compression is a physical-storage choice and does not introduce another logical transcript.
...
atomic admission: turn/start + initial model step if needed + system/user + snapshots + mira/turn-admitted
  ...
  stream to live UI and process-local settlement
