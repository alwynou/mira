...
Backward compatibility is not required in this phase. Change contracts and schemas directly instead of maintaining historical formats. The current schema is documented in [Development](DEVELOPMENT.md); older development libraries are rejected without deletion and only current-schema backup bundles are restored. There are no old-language error aliases or text-based audit-message parsers. Context omissions are typed records localized only in the inspector. Request snapshots are stored only on model attempts, and tool capability uses an explicit `unknown` initial state.
