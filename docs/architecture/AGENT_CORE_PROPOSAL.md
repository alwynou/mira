...
2. **正式会话存储：** 使用追加式 JSONL 日志；原始 UTF-8 字节数不超过 256 KiB 的有效普通文本作为事件字段内、受事务校验和保护的 inline 正文保存，超过该阈值、无法按 UTF-8 解码或超出批次 inline 预算时放入独立受管 external 文件。SQLite 保存业务权威事实和可重建的会话查询投影。日志方案通过持久性、隐私和跨存储一致性验收后，App 才能切换。
...
| 大体积或可清理的会话正文、思考、续接、请求和工具数据 | 按引用选择批次内 inline UTF-8 或具有明确保留分组的受管 external 正文 |
...
当前每个会话使用一个未压缩 JSONL 文件，`SessionJournal` 当前格式版本为 4。每个事件占一个 JSON 对象和一行，字段为 `timestamp`、`type`、`id`、`sequence`、`payload`；时间戳为毫秒精度 ISO 8601。正文不是 UUID 键字典：语义正文节点直接嵌入对应字段，并带有 `id`、`retention_group`、`kind`、`byte_count`、`sha256`，可按字节精确重建的 canonical JSON 正文使用 `json`，其余可 inline 正文使用 `text`；大正文或非 UTF-8 正文仍使用既有 external 存储。compact 引用省略 `sessionID`／`currentBatch`，引用其他批次时带 `source_batch`。

每个逻辑批次由若干事件行组成，并以同一事务最后的 `transaction_commit` 行提交，提交行包含 `format_version: 4`、`id`、`session_id`、`expected_sequence`、`event_count` 和 `checksum`。checksum 是此前该事务全部事件行（包括每行末尾 LF）的精确字节串 SHA-256；只有有效提交行才发布该批次的全部事实和原子接纳结果。事务上限为 8 MiB、256 个事件；元数据上限为 2 MiB，inline 正文／批次预算仍为 256 KiB／2 MiB。正常会话写入严格追加，不能原地修改既有事件；唯一例外是已持久化隐私失效之后为物理擦除 inline 正文而执行的受屏障保护的临时文件重写，该重写重新计算受影响提交校验和、记录偏移和前缀摘要，保留逻辑批次／事件身份与 head。
...
锁适配器保持操作系统资源打开，直到所有写入器关闭；异常进程退出后由系统释放。文件适配器验收前必须确定：每个事务最大字节数（8 MiB）、每批事件数、每事件引用数、正文块大小和有界读取分配；同时规定文件权限、禁止覆盖的发布方式、目录同步和经过验证的文件同步原语。分配和写入前检查上限，不能只在解码完成后检查。单体原始 UTF-8 正文超过 256 KiB、正文不是有效 UTF-8，或本批次 inline 编码总量已耗尽 2 MiB 时，使用 external；inline stage 仅驻留内存并与事务原子发布。

持久化顺序：

1. 按最新已提交状态和当前策略校验命令。
2. 将符合上限的 inline 正文留在内存批次中，或为 external 引用写入并完成持久化屏障、必要时可靠发布新文件名。
...
新正文的发布由同一批次 ID 拥有。external 暂存字节可以成为已可靠发布的文件，但只有已提交引用才能使应用读取该内容；inline 字节只有在完整批次记录提交后才可读。核对结束后才能回收已发布但未引用 external 文件；不确定批次持续固定其候选文件。状态由批次所有权、已提交引用和维护记录推导，不增加第二套独立正文事实源。

首次文件适配器使用 `flock`、`O_NOFOLLOW`、禁止覆盖的 `renamex_np(RENAME_EXCL)` 和文件／目录 `fsync`。external 单体正文上限为 32 MiB；inline 总量上限为 2 MiB；每个 JSONL 事务上限为 8 MiB、最多 256 个事件。扫描按 64 KiB 分块，并在累积事务前检查上限。这里没有承诺 `F_FULLFSYNC` 或所有物理断电条件。
...
恢复时只允许移除可以证明属于未完成最终事务的尾部；事务中已有的完整事件行也随该未提交事务移除。完整非法事件行、完整非法提交行、校验和失败、序号缺口或未知必要事件，应拒绝继续使用相关会话。不能通过截断后续有效内容掩盖损坏。同步和首文件发布必须由经过测试的 macOS 适配器保证，不能从写入成功推断所有断电行为。

若最后一个有效提交事务完整、校验正确而只缺最后 LF，恢复补齐 LF 并同步，保留原事务。完整事件或提交 JSON 但结构／校验非法时拒绝打开；启动恢复只移除未提交的最终事务。只读归档拒绝任何未完成尾部，即使该尾部的最后一行缺 LF。已提交显式物理擦除事件充当可恢复的正文删除清单，启动时继续物理删除；一般 invalidated／逻辑退休事件不能授权物理删除。external 清理失败使打开失败，不能仅屏蔽读取后忽略磁盘残留。inline 清理在 durable invalidation 后通过临时 JSONL 写入、fsync、原子 rename 和目录 fsync 重写事件，重新计算受影响提交 checksum、记录偏移和前缀摘要，保留批次／事件 ID、head 和仍保留正文；重启清理中断临时文件并重建索引／缓存绑定。可信宿主和私有目录是当前边界，不声称抵御恶意同用户进程在检查后替换中间路径。
...
3. 幂等记录会话失效；`invalidatedRetentionGroups` 覆盖所有不可读组，普通 retryCleared 仅逻辑退休并保留物理历史，显式隐私擦除才将相关（包括已退休）组加入 `erasedRetentionGroups`，删除指定 external 会话正文文件并按 inline 重写协议移除批次内正文，再清理派生投影和缓存。
