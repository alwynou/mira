...
2. **正式会话存储：** 使用追加式 JSONL 日志；原始 UTF-8 字节数不超过 256 KiB 的有效普通文本作为批次内、受校验和保护的 inline UTF-8 字典保存，超过该阈值、无法按 UTF-8 解码或超出批次 inline 预算时放入独立受管 external 文件。SQLite 保存业务权威事实和可重建的会话查询投影。日志方案通过持久性、隐私和跨存储一致性验收后，App 才能切换。
...
当前每个会话使用一个未压缩 JSONL 文件，`SessionJournal` 当前格式版本为 3。每一完整行是 `{checksum, record:{batch, payloads:{UUID: exactUTF8String}}}` 的**提交批次**封装；`checksum` 是对精确序列化 `record` JSON 字节计算的 SHA-256。`payloads` 以正文 UUID 为键，值是精确 UTF-8 字符串；用户输入、模型输出、thinking、tool 和 request body 等普通文本均可 inline。正文引用的 `storage` 枚举明确为 `inline` 或 `external`，其 `digest` 对原始 UTF-8 字节计算。原始 UTF-8 字节数小于或等于 256 KiB 且可解码时可以 inline；JSON 字符串转义后的编码字节总量必须不超过 2 MiB，记录 body 必须不超过 8 MiB。无效 UTF-8、超过 256 KiB 或 inline 批次预算耗尽时使用 external。一个批次可以包含多个逻辑事件，读取方不能看到半个批次。正常会话写入严格追加，不能原地修改既有 JSONL 记录；唯一例外是已持久化隐私失效之后为物理擦除 inline 正文而执行的受屏障保护的临时文件重写，该重写重新计算 checksum、记录偏移和前缀摘要，只保留逻辑批次／事件身份与 head。
...
锁适配器保持操作系统资源打开，直到所有写入器关闭；异常进程退出后由系统释放。文件适配器验收前必须确定：每行最大字节数（8 MiB）、每批事件数、每事件引用数、正文块大小和有界读取分配；同时规定文件权限、禁止覆盖的发布方式、目录同步和经过验证的文件同步原语。分配和写入前检查上限，不能只在解码完成后检查。单体原始 UTF-8 正文超过 256 KiB、正文不是有效 UTF-8，或本批次 JSON 转义后的 inline 编码总量已耗尽 2 MiB 时，使用 external；inline stage 仅驻留内存并与批次原子发布。
...
首次文件适配器使用 `flock`、`O_NOFOLLOW`、禁止覆盖的 `renamex_np(RENAME_EXCL)` 和文件／目录 `fsync`。external 单体正文上限为 32 MiB；inline 总量上限为 2 MiB；每条 JSONL 物理记录上限为 8 MiB、最多 256 个事件。扫描按 64 KiB 分块，并在累积单条记录前检查上限。这里没有承诺 `F_FULLFSYNC` 或所有物理断电条件。
...
若最后一个封装完整、校验正确而只缺换行，恢复补齐换行并同步，保留原批次。完整 JSON 但封装／校验非法时拒绝打开；只有不完整尾部允许截断。已提交显式物理擦除事件充当可恢复的正文删除清单，启动时继续物理删除；一般 invalidated／逻辑退休事件不能授权物理删除。external 清理失败使打开失败，不能仅屏蔽读取后忽略磁盘残留。inline 清理在 durable invalidation 后通过临时 JSONL 写入、fsync、原子 rename 和目录 fsync 重写记录，重新计算 checksum、记录偏移和前缀摘要，保留批次／事件 ID、head 和仍保留正文；重启清理中断临时文件并重建索引／缓存绑定。可信宿主和私有目录是当前边界，不声称抵御恶意同用户进程在检查后替换中间路径。
