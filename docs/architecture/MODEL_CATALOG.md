...
The settings templates are OpenAI, Anthropic, Kimi Code, Moonshot, DeepSeek, and OpenRouter. SiliconFlow templates and their unused icon are removed. Protocol/authentication requirements define this scope. LobeHub's much larger provider registry is a reference, not a claim that Mira implements every provider adapter.

Metadata joins use a registered protocol plus exact normalized service endpoint and exact model ID. Display names, substrings, similarly named models and custom proxy origins cannot select an official catalog. Regional metadata remains distinct. `directoryProviders` groups the two Moonshot regional catalogs under one Moonshot service entry, while `matchingProvider` still resolves exact `.cn`/`.ai` endpoints for model capabilities and pricing. A saved connection using either regional endpoint suppresses the same unconfigured Moonshot entry. Kimi Code is a separate service at `https://api.kimi.com/coding/v1` with Coding credentials and model IDs. Editing the API Proxy URL selects the deployment; no automatic region switching, key sharing, or cross-service fallback occurs. Arbitrary custom proxy origins remain unmatched for metadata. A saved connection with a supported service name and protocol stays visible in that service directory after its proxy URL is edited; this presentation grouping grants no catalog capabilities, pricing, or routing authority. Other saved connection data is left untouched and excluded from this six-service directory. User-supplied endpoints and credentials never come from downloaded metadata.

The decoded immutable catalog indexes normalized endpoint/protocol keys and per-provider model IDs. Lookup parses a connection endpoint once, preserves the first matching provider in catalog order and retains the DeepSeek `/v1` alias. These indices contain only bundled reference metadata; credentials and mutable connection snapshots are never cached there.

Explicit provider discovery remains a bounded, cancellable request to that connection's `/models` endpoint. It supplies IDs and optional display names, not account-independent capability proof. Saved models, remote discoveries and bundled entries retain separate source identity in the unified provider model list; duplicate IDs are not offered twice. Catalog models may be unavailable to an account; manually entered deployment IDs remain supported.

## Applied references and overrides

`ModelCatalogMetadata` contains provider/model identity, display name, source URL/revision/date, model task, context/output limits, input/output modalities, tool-call, structured-output, reasoning and continuation hints. No model descriptions or prompts are imported. Publication dates retain the source precision (`YYYY-MM` or `YYYY-MM-DD`), with no date inferred from model IDs or missing values. Provider pricing retains the endpoint-specific pricing contract and is formatted with a plain dollar sign in the UI.
...
## Kimi service reference (2026-09-10)

LobeHub revision `906b10e03029648655e0257bda4f785a9e0973f0` separates [Kimi Code](https://github.com/lobehub/lobehub/blob/906b10e03029648655e0257bda4f785a9e0973f0/packages/model-bank/src/modelProviders/kimiCodingPlan.ts) from [Moonshot](https://github.com/lobehub/lobehub/blob/906b10e03029648655e0257bda4f785a9e0973f0/packages/model-bank/src/modelProviders/moonshot.ts). Mira adopts that service distinction, but uses Kimi Code's [documented OpenAI-compatible endpoint](https://www.kimi.com/code/docs/en/) and [current model IDs](https://www.kimi.com/code/docs/en/kimi-code/models.html), not the older IDs in the inspected LobeHub provider. No separate official Kimi Code China/international endpoint was documented in the reviewed source; its base URL remains editable.

Only the new `kimi-for-coding` provider was normalized from the reviewed models.dev snapshot in this increment. Existing Moonshot snapshots retain their endpoint-specific metadata and receive the common Moonshot display name; unrelated providers are unchanged. Coding membership quota is not recorded as a zero-dollar token tariff. The default connection-test model is `kimi-for-coding` for Kimi Code and `kimi-k2.6` for Moonshot when eligible, while users can select any configured supported text model. Source SHA and retrieval time are recorded per new model.

## Settings controls (2026-09-10)

Models settings now presents the two existing purpose defaults without a duplicate model-pool browser. Global, workspace, and conversation scopes retain the same resolution and revision rules. Draft selections survive unrelated binding refreshes; saves remain explicit, and selecting no local binding applies inheritance only after saving. Provider model context menus retain configuration, removal, and explicit text/tool/JSON probes. UI simplification changes neither model eligibility nor the declared/verified boundary.

Unbound purpose selectors now display a placeholder without a checked menu item. Empty eligible lists show an empty state. Clearing a selected model or restoring inheritance remains an explicit draft edit through a separate menu action, followed by Save; no first model is silently selected.
