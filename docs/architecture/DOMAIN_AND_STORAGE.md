...
新核心的会话权威已采用日志与受管正文，固定前缀、原始用户证据及可重建 SQLite 查询缓存见[会话读取契约](AGENT_SESSION_READS.md)。本页下面的旧会话 SQL 表与备份描述仍对应尚未切换的宿主；P4/P5 将直接替换其调用方和结构，不增加旧数据迁移或兼容读取器。业务事实、业务消费者游标和回执继续属于权威存储，不能随查询投影重建删除。
...
SQLite 保存领域结构化对象、关系、Revision 和索引元数据；大型二进制文件保存在 Blob Store。新核心会话事实由每会话 JSONL 日志承载：正文引用可指向事件字段内的 inline 正文或受管 external 文件，SQLite 会话查询表仅为可重建投影。
