...
Only a source's current successful version enters retrieval. Historical versions remain available through explicit local inspection and authorized exact references. Search normalizes a separate text projection using compatibility normalization and case/width/diacritic folding; original text and offsets remain unchanged.
