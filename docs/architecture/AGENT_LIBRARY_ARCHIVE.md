...
会话头包含最后一批身份和事件序列。正文只复制已提交且仍保留的引用；暂存文件和无引用文件不进入归档。已经失效的正文必须实际不存在，不能因为清单忽略它就通过隐私验证。已提交的失效事实和原引用仍保留在日志中，解释正文缺口。
...
严格日志扫描只读，拒绝残缺尾部、缺失换行、校验和错误、序列间隙、重复批次或冲突正文身份；它不调用普通启动扫描的截断／补换行恢复。正文逐个流式计算摘要，检查实际文件类型、单硬链接、长度和读取前后的文件身份。目录逐层检查，拒绝符号链接、FIFO、路径越界及额外文件；归档副本不容纳暂存／孤儿文件或无意义空目录。
...
关闭核心恢复器、来源读取器、回执适配器、投影及数据库／会话句柄后，恢复器移除本次私有写入器生成的会话锁、索引／检查点、认证材料和待发布正文目录，以及没有规范文件的空模块目录，再严格重验日志、全部保留正文、领域跨引用、暂停策略、附件与确切文件集合。待发布目录的删除不构成正文清理证明：最终只读检查仍拒绝任何无引用文件或已失效正文。文件与目录同步落盘后，以禁止覆盖的原子重命名发布。发布前失败清理本次暂存；发布后父目录同步失败属于确认不确定，已经发布的目标必须保留。
