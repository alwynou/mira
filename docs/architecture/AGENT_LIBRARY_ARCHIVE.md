...
会话头包含最后一批身份和事件序列。归档复制规范 JSONL（包括每批仍保留及逻辑退休的 `payloads` 字典）和已提交且仍保留物理历史的 external 引用文件；retryCleared 等逻辑退休不从归档删除正文，暂存文件和无引用文件不进入归档。归档清单的整份 JSONL 哈希和文件计数包含 inline 字节，但不为 inline 建立独立 manifest 条目；每个 inline 摘要／长度仍由批次内 `SessionPayloadReference` 校验。归档验证器必须根据正文引用核对 inline 原始 UTF-8 哈希、长度和存储枚举，并允许正文缺失仅当最终 durable invalidation 已在日志中；逻辑退休正文仍存在时不得误报为物理缺失，已 erased inline 正文不能仍出现在有效批次记录中。已 erased 的 external 正文必须实际不存在，不能因为清单忽略它就通过隐私验证。已提交的失效事实和原引用仍保留在日志中，解释正文缺口。
...
严格日志扫描只读，拒绝残缺尾部、缺失换行、校验和错误、序列间隙、重复批次或冲突正文身份；它不调用普通启动扫描的截断／补换行恢复。对每批 `payloads` 逐项核对引用、精确 UTF-8 字节、原始字节摘要和总量上限；对 external 正文逐个流式计算摘要，检查实际文件类型、单硬链接、长度和读取前后的文件身份，并按 `invalidated`／`erased` 状态区分可读性和物理缺失。目录逐层检查，拒绝符号链接、FIFO、路径越界及额外文件；归档副本不容纳暂存／孤儿文件或无意义空目录。归档只复制 JSONL 与 external 文件，inline 正文不另写 payload 文件。
...
核心 `AgentLibraryRestoration` 只接收 journal、payload、`AgentBusinessReceipts` 和来源授权接口，没有业务提交、模型目录、工具目录或平台能力。它逐个打开会话，复用执行恢复器核对已有业务回执、结算未完成工具与模型尝试，并记录执行中断。恢复草稿的请求发送目标改为 `.local`，其他冻结身份保持不变；这允许停用连接后仍显示有本地权限的答案和思考，但不会放宽来源修订、工作区、删除或撤权检查。明确撤权抑制可见内容；来源存储不可用则使恢复失败，不伪造完成。
...
关闭核心恢复器、来源读取器、回执适配器、投影及数据库／会话句柄后，恢复器移除本次私有写入器生成的会话锁、索引／检查点、认证材料和待发布正文目录，以及没有规范文件的空模块目录，再严格重验日志、全部未擦除正文（包括逻辑退休历史）、领域跨引用、暂停策略、附件与确切文件集合。待发布目录的删除不构成正文清理证明：最终只读检查仍拒绝任何无引用文件或 `erasedRetentionGroups` 中仍存在的正文；逻辑退休正文应随规范归档保留。文件与目录同步落盘后，以禁止覆盖的原子重命名发布。发布前失败清理本次暂存；发布后父目录同步失败属于确认不确定，已经发布的目标必须保留。
