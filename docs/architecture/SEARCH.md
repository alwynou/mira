...
macOS Memory 已接入本地 Qwen3-Embedding-0.6B 4-bit（固定模型版本、1,024 维 Float32 归一化向量），通过 MLX 使用本机 GPU。没有远程 embedding 回退。Core 只定义 `MemoryEmbeddingService` 与索引工作器，Data 负责 SQLite 派生向量、资格过滤和余弦排序。

- 记忆、来源和发送规则是规范事实；向量仅为可重建索引。修订、内容散列、完整模型身份及索引代次绑定每个待提交结果。
- 当前版本、范围、有效期、发送许可及来源工作区许可先过滤，再执行 top-K。
- 当前固定模型的初步相似度门槛为 0.4；真实模型改写及负例的校验记录见 [semantic fixtures](../engineering/MEMORY_SEMANTIC_FIXTURES.md)。该有限集合不代表通用相关性质量或大规模延迟验收。
- 语义排序优先。完整字面查询或尚未建索引记录最多占一个额外位置；宽泛关键词 OR 不能绕过已建索引记录的语义拒绝。模型不可用时才使用完整词法回退。
- 模型从资料库外的共享目录加载。索引在空闲时运行，查询不触发下载；导出只清理隔离副本中的向量，恢复时重建，遗忘清理派生向量及队列。
