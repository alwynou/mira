...
`ConversationInstructions.default` owns the shared English conversation guidance used by the macOS host and memory model evaluations. It distinguishes a current-conversation acknowledgment from durable persistence and requires the matching successful `memory.remember` result before claiming a new save. Pending, failed, refused, missing or unrelated results cannot serve as that confirmation. The existing journal tool-result continuation carries committed result evidence and the remote-use policy; no new status projection, forced tool call, background wait, output keyword filter or persistence path is introduced. Recalled assertions are evidence for their contents, not for a new save of the current message. These are model instructions, not deterministic enforcement of reply semantics.
