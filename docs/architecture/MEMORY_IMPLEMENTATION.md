...
The foreground `memory.remember` descriptor is revision 4. Its required `enriches` array is empty for an independent save, or contains up to six unique exact `{memory_id, revision}` targets for non-conflicting enrichment. Shared conversation and tool guidance requires searching for the actual facts before an explicit save, including referential requests whose words do not name the facts. The model selects compatible overlapping or contained assertions about the same identified entity and supplies one complete assertion preserving the supported prior facts; shared subject or similarity alone is insufficient. A name-only fact belongs in the targets of a fuller statement that repeats that name; an unrelated nickname preference stays separate. The recall payload exposes structured `memory_id` and `revision` fields, and `memory.search` / `memory.get` resolve concrete targets.

The `quote` parameter remains an exact substring of the current user message. For “remember that,” it quotes the current authorization while the model resolves content from the conversation; concatenated quotations from earlier turns are invalid. An otherwise authorized enrichment target with a different kind gets an actionable, body-free error instructing the model to search again, retain compatible overlapping targets and avoid an independent duplicate. Replacement errors and all scope, disclosure, suppression and revision checks remain strict. These instructions guide semantic selection; the host does not infer natural-language overlap or automatically remove incompatible targets. No invalid proposal writes data, and retrying must not clear all targets merely to bypass a consolidation error.
...
Shared conversation instructions require silent use of memory and natural-language provenance without internal references, including when the user requests sources. Ordinary transcript presentation filters reserved memory references and known receipt identifiers while preserving raw journal evidence. A `memory.remember` receipt proves a committed save, but does not alone establish recorded source authority for that new memory; save acknowledgments omit the receipt's identifiers. Explicit context inspection retains its authorization checks.
