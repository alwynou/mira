...
`memory.remember` 是 `AgentLocalWriteTool`，只准备提案，不自行执行 SQL 写入。其固有策略使用 `.constrained` 与宿主限制共同执行，不能被宿主的允许策略绕过。明确保存由模型按用户语义选择工具，宿主校验当前用户精确摘录、范围与来源抑制，不再用固定前缀或逐字相等识别意图，也不附加记忆专用审批。后台捕获设置不阻断显式保存。来源抑制在准备阶段及最终业务事务内复核，宿主策略仍可拒绝或要求审批。

新工具保存的普通记忆默认可在其授权范围内用于后续模型请求；敏感记忆仍仅限本地。已有记忆的断言去重保留原发送策略，不静默扩大旧记录权限。工具结果报告实际提交状态和发送策略，不能把检索空结果解释成资料库没有保存。需要调整已有记录时通过带修订校验的正常修改用例完成。
...
普通召回只返回当前有效 Active 记忆，排除候选、归档、拒绝、移除、替代、删除、遗忘、未生效和过期内容。macOS 使用本地 Qwen3-Embedding-0.6B 4-bit 模型生成 1,024 维归一化向量，执行语义召回；分词与向量扫描属于 Data，MLX 推理属于宿主，核心只定义端口。查询优先使用语义排序，完整字面匹配及尚未建索引的记录可占一个补充位；已建索引但低于相关性门槛的宽泛关键词命中不能混入结果。模型未准备或推理失败时保留词法召回。

每次记忆写入在同一事务内删除旧向量并排队索引；结果提交校验记忆修订、内容散列、模型身份和索引代次。索引器在前台空闲时运行，查询优先，关闭时排空已派发推理。独立的派生向量表缺失时从当前记忆重建，不更改规范记忆与会话格式；导出副本移除向量，恢复后重建，遗忘清除向量和待执行索引。
