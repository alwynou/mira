...
失败操作先关闭并排空生产者，再提交 `attemptResolved(.failed)`；未结算的部分流保持进程内，硬崩溃时丢失。它的错误正文保存 `AgentModelAttemptFailureRecord`，包含安全诊断、显式重试建议，以及是否接收过任何模型事件。有效用量随失败记录保留。只有确认该批次提交后，执行器才向内核返回重试资格；返回之前已经释放模型调度额度。
