...
用户重试表示**重新回答原问题**。应用在新的接纳批次中追加原始用户身份、冻结路线和新的 execution。既有已提交尝试继续留在日志中；展示投影选择新的回答，不复制或清理未结算的进程内流。

日志确认后，应用仍持有会话预留和冻结目录；重试追加新的执行选择并保留既有已提交尝试，展示投影选择新的回答。模型流在结算前保持进程内，不产生可恢复草稿或隐私清理队列。失败返回 `indeterminate` 时禁止模型／工具派发，通过同一命令的 `reconcileAdmission` 核对原批次；重开文件库只结算已知事实，不自动重新回答。

新回答从原问题重新构建上下文，不拼接旧回复。旧执行保留已提交日志事实、计量及身份元数据；当前来源授权仍在读取和发布前复核。已经提交的任务、记忆等工具业务效果不被撤销；副作用未知的执行仍不允许重试。
...
- **启动：** 读取绑定权威日志前缀，为活动会话加载状态，核对未终态执行的已有工具结果与业务回执，再以中断终态结算。未结算的模型流属于进程内状态，恢复不保存或恢复模型草稿。恢复协调器没有可执行工具目录，也不调用模型或驱动器。恢复未结算时应用保持 `recovering`，可查询和重试结算，不能接纳新任务。
