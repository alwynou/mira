...
`ProviderConnection.isEnabled` and `ModelDescriptor.isEnabled` are independent, required stored fields with checked SQLite mirrors. Schema v9 directly replaces the development schema; earlier libraries and backup formats are rejected intact. Host-created connections saved with Save start inactive. The host activation action tests the selected model before committing an enabled connection; it can also create a new connection directly after a successful test. Enabling a provider never enables its model descriptors or assigns purpose bindings.
...
## 5. Explicit connection checks and activation gate

The macOS configuration presentation model owns a cancellable operation and the draft key, endpoint, and selected test model. It selects the first eligible saved or bundled text model by default and allows another model to be selected. Existing saved API keys remain in Keychain; a replacement key is held only in memory for the explicit draft test. The untitled provider form exposes only key, base/proxy URL, and test controls; protocol and loopback authorization remain the selected template or existing connection values. Arbitrary HTTP endpoints do not become authorized by entering an address. An unmatched custom endpoint can use manually configured text models, including while its connection is inactive.

`AppContainer.saveConnection` gates only a disabled-to-enabled transition (including creation with activation) on a fresh bounded synthetic text probe using the exact draft endpoint, credential, and selected model. Ordinary Save performs local validation and persistence without a network test, keeps the current activation state, and does not require a test-model selection. Ordinary saves do not show a connectivity success result. It checks the saved connection and any saved model/preset revisions before and after the request, checks cancellation before writing, and retains the existing optimistic connection commit. Failure or cancellation before commit neither enables the provider nor replaces a saved key. Deactivation remains immediate and saves only the disabled state, preserving unsaved editor input. For activation, the immutable Keychain installation and rollback/cleanup path runs only after a successful check; ordinary saves use the same credential persistence path without a probe.
