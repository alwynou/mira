...
**版本：** v1.0  
**日期：** 2026-09-05  
**状态：** M0 工程基础已实现；M1 对话、Markdown 与合成能力检测已实现，部分配置与平台验收待补；M2 工具循环基础已实现并进行合成验收。M3–M6 未开始。
...
**交付：** Provider Settings、两类协议、Workspace / Inbox、文本对话、流式 Draft、取消 / 最后失败回合重试、Message 持久化、最小数据备份恢复与模型用量。
...
7. 下载产物的安装、升级、资料库迁移、签名 / 公证以及卸载时数据保留说明完整。没有签名条件时只能标记本机开发可用，不能声称对外交付完成。
...
**目前进度：** 已实现 M1 的可恢复对话、Markdown 与合成能力检测，以及 M2 的多步工具交换、逐次审计、权限检查和限额。M2 使用合成工具完成管线验收，生产注册表为空；M3 / M4 再逐项注册实际记忆与资料工具。用途级路线拆分、Keychain 故障演练及完整平台交互验收仍待补，不将这些缺口记成已完成。
