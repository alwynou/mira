...
The 2026-09-23 [Knowledge management interface proposal](product/KNOWLEDGE_MANAGEMENT_DESIGN.md) adds an interactive source-list/document-reader design for review, with import, versions, permissions and deletion states. The user selected the list/right-preview direction; the completed visual proposal still awaits review. [Browser evidence](engineering/KNOWLEDGE_MANAGEMENT_DESIGN_REVIEW.md) is separate from native implementation, which remains pending. Further Memory work is paused at the user's request; its remaining acceptance gates are retained.
