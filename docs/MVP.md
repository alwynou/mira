...
每个里程碑先补该能力的失败与边界 Fixture，再实现最小纵向路径。M0 不预建整套产品 Schema、所有 Port、图谱、同步、通知或财务模块。以下均为待完成工作，不是进度报告。
...
**目前可以进入 M0：** 产品范围、最低系统、首批协议、文档职责和实施顺序已明确；审阅发现的关键契约已有归属和修正。文档完善不等于实现已证明正确。
