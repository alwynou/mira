...
The [memory recall correction](engineering/MEMORY_RECALL_FIX_VERIFICATION.md) restores normal explicit-save recall and the local Qwen embedding path on the current session architecture. Real-model relevance evidence includes a retained cross-language model limitation; it does not claim perfect recall or port the separate extraction redesign.

The [tool provenance correction](engineering/TOOL_PROVENANCE_VERIFICATION.md) fixes false authorization denials after an earlier conversation turn or a different domain's tool result. Request provenance is authorized independently of tool-owned source plans; domain permissions remain enforced.

The DSH session alignment now uses process-local model streams and canonical settled assistant/tool records. Persistent draft checkpoints and full prepared-request projections have been removed. Frozen session routes also omit metadata provenance; model settings retain the source facts used to resolve effective parameters. Hard process death may lose the unresolved stream prefix; orderly cancellation settles received output once. Focused package, protocol, host and native cancellation/reopen/process-kill checks passed; current evidence and remaining gaps are recorded in [session refactor verification](engineering/AGENT_SESSION_REFACTOR_VERIFICATION.md).
