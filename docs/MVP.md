...
**版本：** v1.0  
**日期：** 2026-09-05  
**状态：** M0 工程基础已实现；M1 首批可恢复对话已实现，M1 尚未完整验收。M2–M6 未开始。
...
**目前进度：** 已进入 M1。工程、两个 Adapter、原生对话与基础存储恢复已有实现；端点能力探测、用途级路线完善、Keychain 故障演练与更完整的交互验收仍是 M1 的后续工作。M2 工具循环未开放，不以已有请求详情页替代 M2 验收。
