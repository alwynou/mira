...
Settings now switches the current main window into a settings mode, with an always-visible category sidebar, grouped detail pages, inline provider configuration, and separate purpose defaults/model pool views. General uses custom dropdowns for display language and a persisted Dark / Light / Follow System display mode. All five category pages share a prominent title and a short localized description. Its sidebar heading, collapse button, and persistent provider/model demo footer are removed. Returning retains the conversation draft, reading intent, and previous sidebar visibility. [Settings layout verification](engineering/SETTINGS_LAYOUT_VERIFICATION.md) records native navigation and localization checks; this changes presentation without adding new model purposes.
