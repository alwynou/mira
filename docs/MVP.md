...
新核心已实现会话日志与正文（当前 v4 追加式 JSONL 按事件记录，正文直接嵌入对应字段，大正文使用受管 external 存储，见[实现与验证记录](engineering/TYPED_SESSION_JOURNAL.md)）、执行循环、作用域模块、模型与工具、调度与审批、来源授权、业务回执、记忆／知识／任务、持久维护及独立资料库恢复。macOS 的会话、工作区、记忆、Data 和 Provider 设置直接使用新服务，完整 App 已恢复构建；没有旧会话 SQL 仓库、旧模型协议或兼容运行时。核心继续只依赖 Foundation，平台能力由宿主模块组装。

The current v4 journal still writes periodic stream checkpoints and has not met the user's requested semantic turn/step structure and presentation ordering. [DSH source research](engineering/DSH_SESSION_LOG_RESEARCH.md) identifies the required runtime and projection changes; that replacement design is not implemented yet.
